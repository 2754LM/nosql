...
public class FollowServiceImpl implements FollowService {
  ...
  public FollowStateResponse exists(Long uid, Long toUserId) {
    return DB.find(UserFollow.class)
            .where()
            .eq("fromUser.id", uid)
  ...
  public List<FollowerResponse> getUserFollowers(Long uid, PageParam pageParam) {
    PagedList<UserFollow> pagedList =
        DB.find(UserFollow.class)
            .fetch("fromUser")
            .where()
            .eq("toUser.id", uid)
            .orderBy("id desc")
    ...
    List<Long> followerIds =
        pagedList.getList().stream().map(uf -> uf.getFromUser().getId()).toList();

    Set<Long> mutualIdSet =
        DB.find(UserFollow.class)
            .where()
            .eq("fromUser.id", uid)
  ...
  public List<FollowingResponse> getUserFollowings(Long uid, PageParam pageParam) {
    ...
    List<Long> followingIds =
        pagedList.getList().stream().map(uf -> uf.getToUser().getId()).toList();
